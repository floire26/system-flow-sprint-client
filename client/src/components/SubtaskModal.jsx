...
import { putTaskAndSubtask } from "../helpers/tasksHelpers";
...
export default function SubtaskModal() {
    ...
    <div className="font-items relative bg-lime-100/60 drop-shadow-lg rounded-lg shadow dark:bg-gray-700 h-4/5">
            <label
                ...
                onClick={() => dispatch(putTaskAndSubtask(task))}
        ...
        <h3 className="font-display text-4xl mb-4 font-medium text-gray-900 dark:text-white">{ `${task.task_name}'s Subtasks` }</h3>   
        <table className="items-center w-full text-sm text-left">
                ...
                <tbody>
                    {
                        task.subtasks.map((st, i) => <SubtaskItem
                            subtaskStatus = {st.subtask_status}
